...
#define MTK_AUTOK_DONE_FILE  "/tmp/mtk_autok_done"
...
int do_host_autok(int id)
{
    ...
    if(nvram_param_count!=0 && nvram_param_count!=param_count){
        system("rm -rf /etc/autok_*");
        system("rm -rf /etc/nvram/APCFG/APRDCL/SDIO");
        close_nvram();
        init_autok_nvram();
        need_to_recover = 1;
    }
    ...
    for(i=0; i<g_vcore_no; i++){
        ...
        LOGD("1-1 Check /etc/autok_[id]_[vol] existence\n");
        // If /etc/autok_id_vol is not exist, it means no tuning
...
int set_autok_done()
{
    int fd = open(MTK_AUTOK_DONE_FILE, O_CREAT|O_TRUNC|O_WRONLY);
    if (fd < 0) {
        LOGE("open mtk autok done file %s failed\n", MTK_AUTOK_DONE_FILE);
        return -1;
    }
    ...
    if (len != strlen(timestamp)) {
        LOGE("write mtk autok done file %s failed\n", MTK_AUTOK_DONE_FILE);
    }
